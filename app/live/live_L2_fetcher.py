...
if __name__ == "__main__":
    ...
    ib_client_id = next((int(arg[7:]) for arg in args if arg.startswith('client=')), 30)

    fetcher = LiveL2Fetcher(worker_type=None, wait_seconds=wait_seconds, continuous=continuous, single_symbol=single_symbol, 
                            ib_disconnect=True, ib_client_id=ib_client_id, paper_trading=paper_trading, remote_ib=remote_ib)
    fetcher.run()
