...
class LiveIBAPIMonitor(live_loop_base.LiveLoopBase):
    def __init__(self, ibkr_path:str=None, wait_seconds:int=None, continuous:bool=True, paper_trading:bool=None):
        super().__init__(wait_seconds=wait_seconds, continuous=continuous, ib_disconnect=True, paper_trading=paper_trading)
        self.ibkr_path = ibkr_path or PATHS.IB_GATEWAY_PATH # Path to IB Gateway or TWS executable

    # Check if IBKR (IB Gateway or TWS) process is running
    def _is_ibkr_running(self):
        ibkr_process = os.path.basename(self.ibkr_path)
        for proc in psutil.process_iter(attrs=['pid', 'name']):
            if ibkr_process in proc.info['name'] or 'TWS' in proc.info['name']:
                print(f"✅ Detected {proc.info['name']} process running with pid {proc.info['pid']}.")
                return True
        print(f"⚠️ No process {ibkr_process} found.")
        return False

    # Health check for IBKR API connection
    def _health_check_ibkr(self):
    ...
    # Restart IBKR application (IB Gateway or TWS)
    def _restart_ibkr(self):
        print("ℹ IBKR process not running, restarting...")
        subprocess.Popen([self.ibkr_path])  # Launch IB Gateway or TWS
        print(f"🔄 IBKR restarted from path: {self.ibkr_path}")
    
    # Login to IBKR application (IB Gateway or TWS)
    def _login_ibkr(self):
        print("🔐 Logging in to IBKR...")
        pass

    # Monitor IBKR API and process
    def monitor(self):
        if self._is_ibkr_running():
            # Check the IBKR API connection
            if not self._health_check_ibkr():
                self._login_ibkr()
        else:
            self._restart_ibkr()
...
if __name__ == "__main__":
    ...
    wait_seconds = next((int(float(arg[5:])) for arg in args if arg.startswith('wait=')), 10)

    fetcher = LiveIBAPIMonitor(wait_seconds=wait_seconds, continuous=continuous, paper_trading=paper_trading)
    fetcher.run()
